...
class SASRec(nn.Module):
    def __init__(self, num_items, max_len, hidden_units, num_blocks, num_heads, dropout_rate, device):
        ...
        # 3. 层归一化
        self.ln_1 = nn.LayerNorm(hidden_units)
        self.ln_2 = nn.LayerNorm(hidden_units)
        ...
        self.final_ln = nn.LayerNorm(hidden_units)
    ...
    def forward(self, log_seqs):
        """
        log_seqs: [batch_size, seq_len], 包含物品 ID 的序列
        返回: [batch_size, seq_len, hidden_units] 的序列表示
        """
        batch_size, seq_len = log_seqs.shape

        # 获取嵌入
        embeddings = self.item_emb(log_seqs)  # [batch, seq, hidden]
...
if __name__ == "__main__":
    ...
    # 初始化模型
    model = SASRec(num_items, max_len, hidden_units, num_blocks, num_heads, dropout_rate, device)
    model.to(device)
    model.train()

    # 模拟输入数据 (随机生成物品 ID，0 代表 padding)
    ...
    # 输出形状应为 [batch_size, seq_len, hidden_units]
